    ...
    def rate_limit(self, shared_limit=True):
        def rate_limit_decorator(f):
            ...
            def wrapper(*args, **kwargs):
                ...
                # Define redis keys
                t = int(time.time())
                closest_minute = t - (t % self.config.get_param("rate_limit_interval"))
                if shared_limit:
                ...
                if self.redis_db.exists(blacklist_key):
                    if request.method == "POST":
                        ...
                        if result['success']:
                            ...
                            # Fail
                            return render_template('blocked.html', g_captcha_site_key=self.config.get_param("recaptcha_site_key")), 429
                    else:
                        return render_template('blocked.html', g_captcha_site_key=self.config.get_param("recaptcha_site_key"), message="Wrong captcha. Try again!"), 429
                ...
                if current and int(current) > self.config.get_param("rate_limit_block_thresh"):
                    self.redis_db.set(blacklist_key, self.config.get_param("rate_limit_block_time"))
                    return render_template('blocked.html', g_captcha_site_key=self.config.get_param("recaptcha_site_key")), 429

                # Check rate limit for a warning
                elif current and int(current) > self.config.get_param("rate_limit_warning_thresh"):
                    ...
                    retry_after = self.config.get_param("rate_limit_interval") - (t - closest_minute)
                    self.logger.info("Hitting rate limit: %s" % (rate_limit_key))
                    resp = jsonify({
                        'code': 429,
                        "message": "Too many requests. Limit %s in %s seconds" % (self.config.get_param("rate_limit_warning_thresh"), self.config.get_param("rate_limit_interval"))
                    })
